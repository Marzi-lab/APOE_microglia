---
title: "Code to generate figures in: The APOE isoforms differentially shape the transcriptomic and epigenomic landscapes of human microglia in a xenotransplantation model of Alzheimer’s disease"
format: html
editor: visual
---
...
# DESeq2 analysis
meta <- data.frame(group, row.names = colnames(atac_f3_counts))
...
# Annotate peaks
txdb <- TxDb.Hsapiens.UCSC.hg38.knownGene
peaks_annot <- as.data.frame(annotatePeak(
...
Figure 1: Transcriptomic and epigenomic profiling of xenotransplanted microglia reveals changes to their regulation in Alzheimer’s disease across the different APOE isoforms. a) Experimental design for xenotransplantation of iPSC-derived human microglia into the brains of AppNL-G-F mice (APOE2 = 5, APOE3 = 5, E4 = 4, APOE KO = 2) and high-quality transcriptomic data across 17 mice (APOE2 = 5, APOE4 = 4, APOE4 = 5, APOE KO = 3), followed by transcriptomic and chromatin accessibility profiling at 12 months. b) Genome tracks showing chromatin accessibility signals of all APOE groups around the Apoe locus. c) Boxplot of expression profiles of APOE, confirming the knockout. d) Stacked barplot of the number of differentially expressed genes (DEGs; FDR < 0.05) identified through pairwise comparisons across the experimental groups. e) Stacked barplot of the number of differentially accessible regions (DARs) and how many of these overlap with the DEGs based on peak-to-gene annotation. 
...
ggplot(apoe, aes(x=group, y=count, fill=group)) + 
  geom_boxplot() + 
  theme_classic() + 
...
## C: chromatin accessibility around APOE locus ##
...
apoe_region <- toGRanges(data.frame(chr="chr19", start=start, end=end))
...
apoe_data <- makeGenesDataFromTxDb(TxDb.Hsapiens.UCSC.hg38.knownGene, karyoplot=kp, plot.transcripts = TRUE, plot.transcripts.structure = TRUE)
...
ggplot(comparisons[!comparisons$diffExpressed == "Not sig",], aes(x=comparison, fill=diffExpressed)) + 
  geom_bar(stat="count") + 
  xlab("") + 
  ylab("Number of DEGs") +
...
# Plot the results
ggplot(gene_counts[gene_counts$assay == "ATAC-seq", ], aes(x = contrast, y = count, fill = assay)) +
...
### Differentially expressed genes are paralleled by changes in chromatin accessibility in their vicinity
...
comparisons$hgnc_symbol[comparisons$Row.names=="102724316"] <- "SVIL-AS1"

## A: E2vsE3 DEGs volcano plot ##

# get top 20 genes for labelling 
top_genes <- head(comparisons[comparisons$comparison == "E2vsE3", ], 20)
...
E2vsE3_volcano <- ggplot(comparisons[comparisons$comparison=="E2vsE3",], 
  ...
  ggtitle("E2 vs E3") + 
  geom_text_repel(data = top_genes, aes(label = hgnc_symbol), box.padding = 0.5, colour="#506777FF", size=5) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "slategrey") +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "slategrey")

## B: E4vsE3 DEGs volcano plot ##

# get top 20 genes for labelling 
top_genes <- head(comparisons[comparisons$comparison == "E4vsE3", ], 20)
...
E4vsE3_volcano <- ggplot(comparisons[comparisons$comparison=="E4vsE3",], 
  ...
  theme(text = element_text(size=16), plot.title = element_text(hjust = 0.5)) + 
  ggtitle("E4 vs E3") + 
  geom_text_repel(data = top_genes, aes(label = hgnc_symbol), box.padding = 0.5, colour="#796F18FF", size=5) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "slategrey") +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "slategrey")

## C: E4vsE2 DEGs volcano plot ##

# get top 20 genes for labelling 
top_genes <- head(comparisons[comparisons$comparison == "E4vsE2", ], 20)
...
E4vsE2_volcano <- ggplot(comparisons[comparisons$comparison=="E4vsE2",], 
  ...
  theme(text = element_text(size=16), plot.title = element_text(hjust = 0.5)) + 
  ggtitle("E4 vs E2") + 
  geom_text_repel(data = top_genes, aes(label = hgnc_symbol), box.padding = 0.5, colour="#3E6248FF", size=5) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "slategrey") +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "slategrey")
...
E2_KO_rrho_cor <- merge(E2vsE3_rrho, KOvsE3_rrho, by = "entrezgene_id")
#cor.test(E2_KO_rrho_cor$rrho_pvalue.x, E2_KO_rrho_cor$rrho_pvalue.y, method="spearman")

## E: RRHO E4 and KO heatmap ##
...
E4_KO_rrho_cor <- merge(E4vsE3_rrho, KOvsE3_rrho, by = "entrezgene_id")
cor.test(E4_KO_rrho_cor$rrho_pvalue.x, E4_KO_rrho_cor$rrho_pvalue.y, method="spearman")

## G: MAGMA gene set analysis with AD GWAS ##

magma_res <- read.csv("data/MAGMA_AD_GWAS_res.csv")
...
# prep data to plot heatmaps of DARs
...
## A: E2vsE3 DAR heatmap with RNA-seq genes ## 
...
## B: E4vsE3 DAR heatmap with RNA-seq genes ##

## A: E4vsE3 DAR heatmap with RNA-seq genes ## 
mat  <- E4vsE3_counts
...
## E: chromatin accessibility around chchd2 ##
...
## F: s-LDSC with AD, ALS, and ASD GWAS ##
...
## B: GO enrichment using E4 genes overlapping DAM ## 
...
gene_module_key <- merge(gene_module_key, gene_ids, by.x="gene", by.y="entrezgene_id")
...
```{r supplementary_figure1}

# APOE-KO vs APOE3 
top_genes <- head(comparisons[comparisons$comparison == "KOvsE3", ], 20)

# need to add labels for the top 20 genes
KOvsE3_volcano <- ggplot(comparisons[comparisons$comparison=="KOvsE3",],
                         aes(x=log2FoldChange,y=-log10(fdr), colour=diffExpressed)) +
  geom_point(size=1.5) + 
  xlim(-10,10) + 
  scale_colour_manual(values=c("#4D6D93FF", "#4D6D93FF", "#9DAFC3FF")) + 
  theme_classic() + 
  ylab("-log10(FDR)") + 
  guides(colour=guide_legend(title="")) + 
  theme(text = element_text(size=16), plot.title = element_text(hjust = 0.5)) + 
  ggtitle("KOvsE3") + 
  geom_text_repel(data = top_genes, aes(label = hgnc_symbol), box.padding = 0.5, colour="#2A3C50FF",
                  size=4)
...
KOvsE3_exc_apoe_volcano <- ggplot(comparisons[comparisons$comparison=="KOvsE3",],
                                  aes(x=log2FoldChange, y=-log10(fdr), colour=diffExpressed)) +
  geom_point(size=1.5) + 
  xlim(-10,10) + 
  scale_colour_manual(values=c("#4D6D93FF", "#4D6D93FF", "#9DAFC3FF")) + 
...
```{r supplementary_figure3}
# corelate with ATAC-seq data
E2vsE3_promoter_peaks <- read.csv("/Volumes/cm1118/projects/chimericmice/live/chimeric_mice/ATACseq/fastq/2021-02-17/HKNNVBBXY/IGFQ001124_hu_8-2-2021_ATACseq/data/sep2023/E2vsE3_promoter_peaks.csv")
E4vsE3_promoter_peaks <- read.csv("/Volumes/cm1118/projects/chimericmice/live/chimeric_mice/ATACseq/fastq/2021-02-17/HKNNVBBXY/IGFQ001124_hu_8-2-2021_ATACseq/data/sep2023/E4vsE3_promoter_peaks.csv")
E4vsE2_promoter_peaks <- read.csv("/Volumes/cm1118/projects/chimericmice/live/chimeric_mice/ATACseq/fastq/2021-02-17/HKNNVBBXY/IGFQ001124_hu_8-2-2021_ATACseq/data/sep2023/E4vsE2_promoter_peaks.csv")
KOvsE3_promoter_peaks <- read.csv("/Volumes/cm1118/projects/chimericmice/live/chimeric_mice/ATACseq/fastq/2021-02-17/HKNNVBBXY/IGFQ001124_hu_8-2-2021_ATACseq/data/sep2023/KOvsE3_promoter_peaks.csv")

# get fold change values 
E2vsE3_atac_log2FoldChange <- E2vsE3_promoter_peaks[,c("log2FoldChange", "geneId")]
E4vsE3_atac_log2FoldChange <- E4vsE3_promoter_peaks[,c("log2FoldChange", "geneId")]
E4vsE2_atac_log2FoldChange <- E4vsE2_promoter_peaks[,c("log2FoldChange", "geneId")]
KOvsE3_atac_log2FoldChange <- KOvsE3_promoter_peaks[,c("log2FoldChange", "geneId")]

# E2vsE3 
E2vsE3_atac_log2FoldChange <- E2vsE3_atac_log2FoldChange %>%
  group_by(geneId) %>%
  dplyr::summarise(median_log2FoldChange = median(log2FoldChange, na.rm = TRUE))

E2vsE3_cor <- merge(E2vsE3_rna_sig, E2vsE3_atac_log2FoldChange, by.x="entrezgene_id", by.y="geneId")

E2vsE3_cor_res <- cor.test(E2vsE3_cor$log2FoldChange, E2vsE3_cor$median_log2FoldChange, 
                           method = "pearson")
...
ggsave("E2vsE3_expression_vs_accessibility_logFC.pdf", height=4, width = 5)

# E4vsE2 
E4vsE2_atac_log2FoldChange <- E4vsE2_atac_log2FoldChange %>%
  group_by(geneId) %>%
  dplyr::summarise(median_log2FoldChange = median(log2FoldChange, na.rm = TRUE))

E4vsE2_cor <- merge(E4vsE2_rna_sig, E4vsE2_atac_log2FoldChange, by.x="entrezgene_id", by.y="geneId")
...
ggsave("E4vsE2_expression_vs_accessibility_logFC.pdf", height=4, width = 5)

# E4vsE3 
E4vsE3_atac_log2FoldChange <- E4vsE3_atac_log2FoldChange %>%
  group_by(geneId) %>%
  dplyr::summarise(median_log2FoldChange = median(log2FoldChange, na.rm = TRUE))

E4vsE3_cor <- merge(E4vsE3_rna_sig, E4vsE3_atac_log2FoldChange, by.x="entrezgene_id", by.y="geneId")
...
ggsave("E4vsE3_expression_vs_accessibility_logFC.pdf", height=4, width = 5)

# KOvsE3 
KOvsE3_atac_log2FoldChange <- KOvsE3_atac_log2FoldChange %>%
  group_by(geneId) %>%
  dplyr::summarise(median_log2FoldChange = median(log2FoldChange, na.rm = TRUE))

KOvsE3_cor <- merge(KOvsE3_rna_sig, KOvsE3_atac_log2FoldChange, by.x="entrezgene_id", by.y="geneId")
...
ggsave("KOvsE3_expression_vs_accessibility_logFC.pdf", height=4, width = 5)

```

```{r supplementary_figure4}
